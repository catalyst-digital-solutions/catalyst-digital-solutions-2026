...
- **Assets:** `public/assets/trades/` — Super J proof gallery (identity, socials, character 3-pose row, mark carousel, wrap, flatlay v2, OG). Mark carousel slides: `j-shield-emblem-social_*` + `j-shield-emblem-*-bg.webp`. Character poses: `mascot-thumbs-up.jpeg`, `mascot-arms-crossed.jpeg`, `super-j-pointing-pose.png`. Flatlay: `super-j-flatlay-v2.jpg`. OG: `super-j-opengraph-image.png`. Favicon: `j-shield-emblem_circle-favicon.png` (wired in `src/app/trades/page.tsx` metadata). Source: Super J `public/brand/`.
...
- **Copy/layout notes (live):** Package purple card mirrors the full offer card: price from `slots`, payment/ownership line, slot dots + counter, next-rung line, $149/mo after-launch block, Book CTA, no-pressure closer (no strikethrough agency comparison). Price ladder under that card — “Only 5 available at this price.” + promotion body. Demos (“We do more than just mascots”) sit above The Package. Gallery order: identity → socials → avatars → character poses → mark carousel → wrap → flatlay → OG. Mark carousel: swipe on image; arrows/dots stop autoplay. How-it-works in FAQ. Final CTA: “How does your brand compare?” README in `branding-and-website-landing-page/` supersedes the PRD where they conflict; Mario’s live edits take priority after ship
