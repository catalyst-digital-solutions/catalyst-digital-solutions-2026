...
- **Copy/layout notes (live):** Price ladder under package card — title “Only 5 available at this price.” + promotion body line before Book CTA. Gallery order: identity → socials → avatars → character poses → mark carousel → wrap → flatlay → OG. Mark carousel: swipe on image; arrows/dots stop autoplay (controls outside swipe capture). How-it-works in FAQ. Demo grid CLICK/TAP TO SEE THESE LIVE. Final CTA: “How does your brand compare?” README in `branding-and-website-landing-page/` supersedes the PRD where they conflict; Mario’s live edits take priority after ship
